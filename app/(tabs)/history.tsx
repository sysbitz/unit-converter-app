...
export default function HistoryScreen() {
	...
	function confirmClear() {
		Alert.alert("ইতিহাস মুছুন", "সব রূপান্তর ইতিহাস মুছে ফেলবেন?", [
				...
				<View style={styles.cardContent}>
					<View style={styles.cardTop}>
						<Text style={[styles.categoryLabel, { color }]}>
							{getCategoryLabel(item.category_id, item.category_name)}
						</Text>
						<Text style={styles.dateText}>{formatDate(item.created_at)}</Text>
					</View>
					<Text style={styles.conversionText}>
						{toBengaliDigits(formatResult(item.input_value))}{" "}
						{item.from_unit_name}
						{"  →  "}
						{toBengaliDigits(formatResult(item.output_value))}{" "}
						{item.to_unit_name}
					</Text>
				</View>
			</View>

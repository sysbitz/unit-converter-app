...
import { UNIT_CATEGORIES, UnitCategory, Unit, convert, formatResult, toBengaliDigits } from '@/constants/units';
...
export default function ConverterScreen() {
  ...
  const result = inputValue ? convert(parseFloat(inputValue), fromUnit, toUnit) : null;
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          <TouchableOpacity onPress={() => setSelectingFor(null)} style={styles.backBtn}>
            <Feather name="arrow-left" size={22} color={colors.text} />
          </TouchableOpacity>
          <Text style={styles.headerTitle}>
            {selectingFor === 'from' ? 'থেকে একক বেছে নিন' : 'যে এককে রূপান্তর করবেন'}
          </Text>
        </View>
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              style={[
                styles.unitOption,
                (selectingFor === 'from' ? fromUnit : toUnit).id === item.id && styles.unitOptionSelected,
              ]}
  ...
  return (
    <View style={styles.container}>
      {/* Header */}
      <View style={styles.header}>
        <Text style={styles.headerTitle}>ইউনিট রূপান্তর</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.swapRow}>
            ...
            <TouchableOpacity style={[styles.swapBtn, { backgroundColor: selectedCategory.color }]} onPress={handleSwap}>
          ...
          <View style={styles.unitBlock}>
            ...
            >
              ...
              <View style={[styles.symbolBadge, { backgroundColor: selectedCategory.color + '20' }]}>
                <Text style={[styles.symbolText, { color: selectedCategory.color }]}>{toUnit.symbol}</Text>
              </View>
              <Feather name="chevron-down" size={16} color={colors.textSecondary} />
            </TouchableOpacity>

            <View style={styles.resultBox}>
              <Text style={[styles.resultText, { color: selectedCategory.color }]}>
                {result !== null
                  ? toBengaliDigits(formatResult(result))
                  : '—'}
              </Text>
            </View>
          </View>
        ...
        {result !== null && inputValue && (
          <View style={styles.formulaCard}>
            ...
            <Text style={styles.formulaText}>
              {toBengaliDigits(inputValue)} {fromUnit.symbol} = {toBengaliDigits(formatResult(result))} {toUnit.symbol}
            </Text>
          </View>
        )}
        ...
        {inputValue ? (
          <View style={styles.allUnitsCard}>
            {selectedCategory.units.map(unit => (
              <View key={unit.id} style={styles.allUnitRow}>
                ...
                <Text style={[styles.allUnitValue, { color: selectedCategory.color }]}>
                  {toBengaliDigits(formatResult(convert(parseFloat(inputValue), fromUnit, unit)))} {unit.symbol}
                </Text>
              </View>
            ))}
          </View>
          ...
          <View style={styles.emptyHint}>
            <Text style={styles.emptyHintText}>উপরে একটি মান লিখুন সকল রূপান্তর দেখতে</Text>
          </View>
        )}
...
const makeStyles = (colors: any, isDark: boolean) => StyleSheet.create({
  ...
  headerTitle: { fontFamily: 'Hind-Siliguri-Bold', fontSize: 20, color: colors.text, flex: 1 },
  ...
  categoryChipText: { fontFamily: 'Hind-Siliguri-Medium', fontSize: 13, color: colors.textSecondary },
  card: {
  ...
  unitSelectorLabel: { fontFamily: 'Hind-Siliguri-Regular', fontSize: 12, color: colors.textTertiary },
  unitSelectorValue: { fontFamily: 'Hind-Siliguri-SemiBold', fontSize: 16, color: colors.text },
  symbolBadge: {
    paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8,
    backgroundColor: colors.primaryLight, marginLeft: 'auto',
  },
  symbolText: { fontFamily: 'Hind-Siliguri-Medium', fontSize: 13, color: colors.primary },
  valueInput: {
    backgroundColor: colors.surfaceSecondary, borderRadius: 12,
    paddingHorizontal: 16, paddingVertical: 14,
    fontFamily: 'Hind-Siliguri-Regular', fontSize: 24, color: colors.text,
    borderWidth: 1.5, borderColor: colors.border,
  },
  swapRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, gap: 12 },
  ...
  formulaText: { fontFamily: 'Hind-Siliguri-Regular', fontSize: 14, color: colors.textSecondary },
  sectionTitle: {
    fontFamily: 'Hind-Siliguri-SemiBold', fontSize: 16, color: colors.text,
  ...
  allUnitName: { fontFamily: 'Hind-Siliguri-Regular', fontSize: 14, color: colors.textSecondary },
  allUnitValue: { fontFamily: 'Hind-Siliguri-SemiBold', fontSize: 14 },
  emptyHint: {
    marginHorizontal: 16, backgroundColor: colors.surface,
    borderRadius: 16, padding: 24, alignItems: 'center',
  },
  emptyHintText: { fontFamily: 'Hind-Siliguri-Regular', fontSize: 14, color: colors.textTertiary },
  unitOption: {
  ...
  unitOptionSelected: { borderColor: colors.primary, backgroundColor: colors.primaryLight },
  unitOptionSymbol: { fontFamily: 'Hind-Siliguri-Bold', fontSize: 16, color: colors.primary, width: 48 },
  unitOptionName: { fontFamily: 'Hind-Siliguri-Medium', fontSize: 15, color: colors.text, flex: 1 },
});
